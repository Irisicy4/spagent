"""
MapAnything 3D Reconstruction Tool

This module contains the MapAnythingTool that wraps
MapAnything 3D reconstruction functionality for the SPAgent system.
"""
...
class MapAnythingTool(Tool):
    """Tool for 3D reconstruction from images using MapAnything"""
    
    def __init__(self, use_mock: bool = True, server_url: str = "http://localhost:20022", mode='inference'):
        """
        Initialize MapAnything tool
        
        Args:
            use_mock: Whether to use mock client for testing
            server_url: URL of the MapAnything server
            mode: 'inference' or 'train' mode
        """
        super().__init__(
            ...
            description=(
                ...
                "using MapAnything to generate point clouds and visualizations from CUSTOM viewing angles.\n\n"
                ...
                "Note that default camera_view is false. You must output camera_view = true if you want to set ego-view. "
                "If you want to set global-view, you must output camera_view = false.\n\n"
    ...
    def _init_client(self):
        """Initialize the MapAnything client"""
        if self.use_mock:
            try:
                from external_experts.mapanything.mock_mapanything_service import MockMapAnythingService
                self._client = MockMapAnythingService()
                logger.info("Using mock MapAnything service")
            except ImportError:
                # Create a simple mock client
                class SimpleMockMapAnything:
                    def infer_from_images(self, image_paths, azimuth_angle=None, elevation_angle=None, **kwargs):
                        ...
                        result = {
                            "success": True,
                            "ply_filename": f"mapanything_result_{image_name}.ply",
                            "points_count": 50000,
                            "camera_views": []
                        }
                        ...
                        if azimuth_angle is not None and elevation_angle is not None:
                            ...
                            draw.rectangle([20, 20, 320, 120], fill='white', outline='black', width=2)
                            draw.text((30, 30), f"Azimuth: {azimuth_angle}°", fill='black')
                            draw.text((30, 50), f"Elevation: {elevation_angle}°", fill='black')
                            draw.text((30, 70), "MapAnything Mock Render", fill='gray')
                ...
                self._client = SimpleMockMapAnything()
                logger.info("Using simple mock MapAnything service")
        else:
            try:
                from external_experts.mapanything.mapanything_client import MapAnythingClient
                self._client = MapAnythingClient(server_url=self.server_url)
                logger.info(f"Using real MapAnything service at {self.server_url}")
            except ImportError as e:
                logger.error(f"Failed to import real MapAnything client: {e}")
                raise
    ...
    def call(
        ...
        """
        Execute 3D reconstruction using MapAnything
        ...
        try:
            ...
            logger.info(f"Running MapAnything 3D reconstruction on images: {image_path}")
            ...
            if result and result.get('success'):
                logger.info("MapAnything 3D reconstruction completed successfully")
                ...
                description = (
                    f"MapAnything tool has completed 3D reconstruction, generating a point cloud visualization "
                ...
                logger.error(f"MapAnything 3D reconstruction failed: {error_msg}")
                return {
                    "success": False,
                    "error": f"MapAnything 3D reconstruction failed: {error_msg}"
                }
                
        except Exception as e:
            logger.error(f"MapAnything tool error: {e}")
            return {
                "success": False,
                "error": str(e)
            }
    ...
    def _check_cache(self, image_path: str, azimuth_angle: float, elevation_angle: float, 
        ...
        try:
            ...
            cache_filename = f"mapanything_{scene_id}_azim{azimuth_angle:.1f}_elev{elevation_angle:.1f}{suffix}.png"
            cache_path = os.path.join(output_dir, cache_filename)
            
            # Check if cache file exists
            if not os.path.exists(cache_path):
                logger.debug(f"No cache found: {cache_path}")
                return None
            ...
            return ToolResult(
                ...
                payload=payload,
                description=(
                    f"Using cached MapAnything visualization from previous reconstruction "
    ...
    def _save_generated_images(self, result: Dict[str, Any], image_path: str, 
        ...
        """
        Save generated images from MapAnything result and return the path to the first saved image
        
        Args:
            result: MapAnything reconstruction result
            image_path: Original input image path for naming
            rotation_reference_camera: Reference camera index (1-based)
            camera_view: Whether using camera view mode
        ...
        try:
            ...
            camera_views = result.get('camera_views', [])
            if not camera_views:
                logger.warning("No camera views found in MapAnything result")
                return None
            ...
            for i, view_data in enumerate(camera_views):
                try:
                    ...
                    img_filename = f"mapanything_{scene_id}_azim{azimuth:.1f}_elev{elevation:.1f}{suffix}.png"
                    ...
                    if "image" in view_data:
                        ...
                        logger.info(f"Saved MapAnything generated image: {img_path}")
                    
                except Exception as e:
                    logger.error(f"Failed to save camera view {i}: {e}")
                    continue
            
            # Return the first saved image path
            if saved_images:
            ...
            logger.error(f"Error saving MapAnything generated images: {e}")
            return None
