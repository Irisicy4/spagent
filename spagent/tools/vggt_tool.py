"""
VGGT 3D Reconstruction Tool

This module contains the VGGTTool that wraps
VGGT 3D reconstruction functionality for the SPAgent system.
"""
...
class VGGTTool(Tool):
    """Tool for 3D reconstruction from multiple images using VGGT"""
    ...
    def __init__(self, use_mock: bool = True, server_url: str = "http://localhost:20022", mode='inference'):
        ...
        super().__init__(
            ...
            description=(
                ...
                "object rotation, or directional motion analysis. It performs 3D reconstruction from multiple images "
                "using VGGT (Visual Geometry Grounded Transformer) to generate point clouds and visualizations "
                "from CUSTOM viewing angles.\n\n"
    ...
    def _init_client(self):
        ...
        if self.use_mock:
            try:
                from external_experts.VGGT.mock_vggt_service import MockVGGTService
                self._client = MockVGGTService()
                logger.info("Using mock VGGT service")
            except ImportError:
                # Create a simple mock client
                class SimpleMockVGGT:
                    def infer_from_images(self, image_paths, azimuth_angle=None, elevation_angle=None, **kwargs):
                        ...
                        result = {
                            "success": True,
                            "ply_filename": f"vggt_result_{image_name}.ply",
                            "points_count": 50000,
                            "camera_views": [],
                            "camera_poses": []
                        }
                        ...
                        if azimuth_angle is not None and elevation_angle is not None:
                            ...
                            draw.text((30, 70), "VGGT Mock Render", fill='gray')
                ...
                self._client = SimpleMockVGGT()
                logger.info("Using simple mock VGGT service")
        else:
            try:
                from external_experts.VGGT.vggt_client import VGGTClient
                self._client = VGGTClient(server_url=self.server_url)
                logger.info(f"Using real VGGT service at {self.server_url}")
            except ImportError as e:
                logger.error(f"Failed to import real VGGT client: {e}")
                raise
    ...
    def call(
        ...
        """
        Execute 3D reconstruction using VGGT
        ...
        try:
            ...
            logger.info(f"Running VGGT 3D reconstruction on images: {image_path}")
            ...
            if result and result.get('success'):
                logger.info("VGGT 3D reconstruction completed successfully")
                ...
                description = (
                    f"VGGT tool has completed 3D reconstruction, generating a point cloud visualization "
                ...
                logger.error(f"VGGT 3D reconstruction failed: {error_msg}")
                return {
                    "success": False,
                    "error": f"VGGT 3D reconstruction failed: {error_msg}"
                }
                
        except Exception as e:
            logger.error(f"VGGT tool error: {e}")
            return {
                "success": False,
                "error": str(e)
            }
    ...
    def _check_cache(self, image_path: str, azimuth_angle: float, elevation_angle: float, 
        ...
        try:
            ...
            cache_filename = f"vggt_{scene_id}_azim{azimuth_angle:.1f}_elev{elevation_angle:.1f}{suffix}.png"
            ...
            return ToolResult(
                ...
                payload=payload,
                description=(
                    f"Using cached VGGT visualization from previous reconstruction "
    ...
    def _save_generated_images(self, result: Dict[str, Any], image_path: str, 
        ...
        """
        Save generated images from VGGT result and return the path to the first saved image
        
        Args:
            result: VGGT reconstruction result
            image_path: Original input image path for naming
            rotation_reference_camera: Reference camera index (1-based)
            camera_view: Whether using camera view mode
        ...
        try:
            ...
            camera_views = result.get('camera_views', [])
            if not camera_views:
                logger.warning("No camera views found in VGGT result")
                return None
            ...
            for i, view_data in enumerate(camera_views):
                try:
                    ...
                    img_filename = f"vggt_{scene_id}_azim{azimuth:.1f}_elev{elevation:.1f}{suffix}.png"
                    ...
                    if "image" in view_data:
                        ...
                        logger.info(f"Saved VGGT generated image: {img_path}")
            ...
            logger.error(f"Error saving VGGT generated images: {e}")
            return None
