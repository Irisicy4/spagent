...
| **FaceDetection** | `FaceDetectionTool` | Frontal Face Detection | CPU-only face detection via OpenCV Haar cascade (ships with opencv — no download); pixel-xyxy boxes + annotated image; zero faces is a valid finding | Local (no server) | `image_path`, `min_neighbors`(optional) |
| **Qwen2.5-VL** | `QwenVLTool` | Vision-Language Detection | Cloud VLM detection (qwen-vl-max): `ref_detection` (find objects matching a description) and `reasoning_detection` (find objects answering a question); requires an API key | API (no server) | `image_path`, `text_prompt`, `task`(optional) |
